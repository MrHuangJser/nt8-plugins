...
namespace NinjaTrader.NinjaScript.DrawingTools
{
    /// <summary>
    /// LongPosition - 多头画图工具
    /// 类似 TradingView 的多头工具，支持四锚点绘制
    /// 左上(Target)、左中(Entry)、左下(Stop)、右中(宽度控制)
    /// 自动计算仓位并更新 Chart Trader 手数
    /// </summary>
    public class LongPosition : DrawingTool
    {
        #region Variables
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = @"多头画图工具 - 类似 TradingView Long Position Tool";
                Name = "LongPosition";
        ...
        public override Point[] GetSelectionPoints(ChartControl chartControl, ChartScale chartScale)
        {
            ...
            return new[]
            {
                ...
                new Point(entryPoint.X, stopPoint.Y),     // 左下 Stop
                new Point(entryPoint.X, targetPoint.Y),   // 左上 Target
                                          ...
                                          ChartScale chartScale, ChartAnchor dataPoint)
        {
            switch (DrawingState)
            {
                case DrawingState.Building:
                    ...
                    {
                        // 第二次点击 - 设置 Stop
                        // Stop 只取价格(Y)，时间(X)保持与 Entry 一致
                        StopAnchor.Price = Math.Min(dataPoint.Price, EntryAnchor.Price - tickSize);
                    ...
                    else if (TargetAnchor.IsEditing)
                    {
                        ...
                        TargetAnchor.Price = Math.Max(dataPoint.Price, EntryAnchor.Price + tickSize);
                        TargetAnchor.Time = EntryAnchor.Time;
            ...
            switch (DrawingState)
            {
                case DrawingState.Building:
                    ...
                    else if (TargetAnchor.IsEditing)
                    {
                        ...
                        // Target 只跟随鼠标 Y（价格），限制在 Entry 上方
                        if (mousePrice > entryPrice)
                        {
                                  ...
                                  Point entryPoint, Point stopPoint, Point targetPoint)
        {
            ...
            // Target Zone (绿色) - Entry 到 Target 之间
                                      ...
                                      float x, float y)
        {
            ...
            {
                var rect = new SharpDX.RectangleF(x, y + 10, panelWidth, panelHeight);
                ...
                FontSize))
            {
                    ...
                    SharpDX.Color.White))
                {
                    for (int i = 0; i < lines.Length; i++)
                    {
                        ...
                        {
                            RenderTarget.DrawTextLayout(
                                new SharpDX.Vector2(x + 5, y + 15 + i * lineHeight),
                                textLayout,
                                textBrush);
                        }
                    }
