...
namespace NinjaTrader.NinjaScript.DrawingTools
{
    /// <summary>
    /// LongPosition - 多头画图工具
    /// 类似 TradingView 的多头工具，支持三点绘制（Entry/Stop/Target）
            ...
            switch (DrawingState)
            {
                case DrawingState.Building:
                    ...
                    else if (StopAnchor.IsEditing)
                    {
                        ...
                        TargetAnchor.Price = entryPrice + risk * 2; // 默认 2R
                    }
                                          ...
                                          ChartScale chartScale, ChartAnchor dataPoint)
        {
            ...
            switch (DrawingState)
            {
                case DrawingState.Building:
                    ...
                    {
                        // Stop 跟随鼠标，但限制在 Entry 下方（多头）
                        double entryPrice = EntryAnchor.Price;
                        double mousePrice = dataPoint.Price;

                        // 多头: Stop 必须在 Entry 下方
                        ...
                        {
                            // 如果鼠标在 Entry 上方，将 Stop 设置为略低于 Entry
                    ...
                    {
                        // Target 跟随鼠标，但限制在 Entry 上方（多头）
                        double entryPrice = EntryAnchor.Price;
                        double mousePrice = dataPoint.Price;

                        // 多头: Target 必须在 Entry 上方
                        if (mousePrice > entryPrice)
                        {
                            dataPoint.CopyDataValues(TargetAnchor);
                        }
                        ...
                        {
                            // 如果鼠标在 Entry 下方，将 Target 设置为略高于 Entry
